...
- **Clarity와 Graphite Clarity로 Editor를 더 쉽게 읽을 수 있습니다.** 두 새
  테마는 모던 카드와 레거시 행에서 다크·라이트 모드를 모두 지원하며, 레이블,
  포커스 상태, 비활성 행, 컨트롤, 노브 값이 뚜렷하게 보입니다. Graphite는 짙은
  회색의 계기판 스타일을 쓰고, Clarity는 가장 높은 대비를 우선합니다.
- **레거시 VST3 채널 채우기 컨트롤이 깔끔하게 줄바꿈됩니다.** 긴 채널
  레이아웃에서도 레이블이나 컨트롤이 겹치지 않습니다.
...
- **VST 분석기 패널이 현재 마이크 입력을 다시 보여 줍니다.** Bertom
  ...
  받고, 알려진 Bertom 네이티브 패널 예외는 계속 보호됩니다.
- **영향받은 Windows 호스트에서 자동 감지 설치 프로그램이 Program Files를
  찾습니다.** x86 프런트도어는 Program Files 알려진 폴더 조회를 사용할 수 없을
  때 보호된 네이티브 레지스트리 값으로 폴백하므로, 다운로드가 시작되기 전에
  실패하지 않습니다. 릴리스 테스트 도우미도 기본으로 시스템 전체 MSI를
  선택합니다.
