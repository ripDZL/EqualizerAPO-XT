...
int selectorWidth(bool output)
{
	...
	const qreal roleWidth = qMax(roleMetrics.horizontalAdvance(output ? QStringLiteral("OUT") : QStringLiteral("IN")),
		roleMetrics.horizontalAdvance(output ? VSTBusStrip::tr("Out") : VSTBusStrip::tr("In")));
	return qRound(GUIHelper::scale(12.0) + roleWidth + GUIHelper::scale(4.0)
		+ widestValue + GUIHelper::scale(9.0));
}

int hMargin() { return GUIHelper::scale(4.0); }
int vMargin() { return GUIHelper::scale(3.0); }
int jointWidth() { return GUIHelper::scale(14.0); }
int verdictGap() { return GUIHelper::scale(6.0); }
}

VSTBusSelector::VSTBusSelector(bool output, QWidget* parent)
	: QWidget(parent), output(output)
{
	setObjectName(output ? QStringLiteral("VSTBusSelectorOut") : QStringLiteral("VSTBusSelectorIn"));
	setFocusPolicy(Qt::StrongFocus);
	setCursor(Qt::PointingHandCursor);
	setAccessibleName(output ? VSTBusStrip::tr("VST3 output bus layout")
...
void VSTBusSelector::openMenu()
{
	...
	if (chosen == nullptr)
		return;
	setBusLayout(static_cast<VST3BusLayout>(chosen->data().toInt()));
	emit busLayoutPicked(current);
}
...
{
	if (verdictText == text && verdictInputText.isEmpty() && verdictOutputText.isEmpty() && verdictTone == tone)
		return;
	verdictText = text;
...
{
	if (verdictText.isEmpty() && verdictInputText == input && verdictOutputText == output && verdictTone == tone)
		return;
	verdictText.clear();
...
{
	int width = hMargin() * 2 + inputSelector->sizeHint().width() + jointWidth() + outputSelector->sizeHint().width();
...
VstBusFrameState VSTBusStrip::frameState() const
{
	...
	state.verdictRect = QRect(state.outputRect.right() + 1 + verdictGap(), state.outputRect.top(),
		qMax(0, width() - hMargin() - (state.outputRect.right() + 1 + verdictGap())), state.outputRect.height());
...
{
	const bool hasText = !verdictText.isEmpty() || !verdictInputText.isEmpty() || !verdictOutputText.isEmpty();
	if (!hasText)
	...
	int text = !verdictInputText.isEmpty() || !verdictOutputText.isEmpty()
		? qRound(metrics.horizontalAdvance(verdictInputText) + metrics.horizontalAdvance(verdictOutputText)) + GUIHelper::scale(16.0)
		: qRound(metrics.horizontalAdvance(verdictText)) + GUIHelper::scale(5.0);
	return qMin(GUIHelper::scale(150.0), GUIHelper::scale(18.0) + text);
}
