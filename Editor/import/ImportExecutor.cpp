...
bool hasExpectedVST3Module(const QString& sourceRoot)
{
    const QString moduleDirectory = QDir(sourceRoot).absoluteFilePath(
                    ...
                    int* copiedFiles, qint64* copiedBytes, ExecutionResult& result)
{
    ...
    if (!hasExpectedVST3Module(sourceRoot))
    {
        result.success = false;
        result.errors.append(QObject::tr("VST3 bundle has no %1 module: %2")
            .arg(vst3PlatformDirectory(), sourceRoot));
        return false;
    }
