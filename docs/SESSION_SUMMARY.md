...
- 2026-08-13: completed the upstream #265 VST3 editor port plus #271 Matrix body-band visual follow-up locally on `codex/upstream-265-vst3-layouts`. It adds native per-skin Input/Output layout controls, VST3 negotiated-layout diagnostics, VST2-safe fallback, VST3 fixture gallery coverage, and protected serialization of the fork's preview/device-routing behavior. AVX2 Editor rebuilt; HybridConv 1635 (VST3 121), EditorLogic 3555, EngineOrchestration 1221, AudioRegression 31/31, source sync, and diff check passed. The local self-test and forced-offscreen gallery now stall before output even from a disposable console-subsystem copy, so do not use those as acceptance evidence; push the isolated branch for hosted CI, then perform the real multichannel VST3 host test before any `beta`/`main` promotion. No active runtime changed. Upstream is at `57bf44e`; defer its version/design-sync/docs-only commits to a separate later batch.
- 2026-08-13: pushed the tree-identical #265/#271 implementation as `740a86e` on the isolated branch and hosted Build EqualizerAPO run `31711097531` is fully green: all native variants, offscreen UI gates, Pester, cppcheck, and cross-variant comparison passed. Its initial AVX2 5.497 s skin-switch outlier did not reproduce on the unchanged failed-job retry (worst 2160 ms, same 5000 ms limit). Installed the matching 30-file AVX-512 artifact at `C:\Users\Admin\AppData\Local\EqualizerAPO-XT-x64-avx512\current` after a full-current backup at `artifacts\install-backups\pre-upstream-265-vst3-layouts-ci-31711097531-20260813-161800`; every payload file hash-verified and installed VST self-test passed. User reports the real `Input Stereo Output 7.1` VST3 test appears to work. User-approved fast-forward promoted this integration to `beta`; `main` remains unchanged.
- 2026-08-11 VST3 follow-up: `VSTPluginLibrary` now tolerates optional factory `kNotImplemented`; `VSTPluginInstance` preserves all auxiliary arrangements and accepts a compatible layout read back after `setBusArrangements` returns `kResultFalse`. Added factory/sidechain/adaptation fixtures; `HybridConvTests.exe` passed 1635 checks (VST3 119). The local real-plugin probe passed FabFilter Pro-Q 4 Stereo->Stereo and 7.1->7.1. Local AVX2 `EqualizerAPO.dll` and `build-Editor-x64-vst3-layouts\release\Editor.exe` rebuilt; `Editor.exe --selftest-vst` stalled and was stopped. No active runtime changed. Next: explicit-install manual real-audio test; do not promote to `beta`/`main` yet.
- 2026-08-11: installed the CI-tested #265 x64-avx512 payload into the active per-user runtime (`C:\Users\Admin\AppData\Local\EqualizerAPO-XT-x64-avx512\current`), not the obsolete Program Files copy. Seven changed binaries were backed up at `artifacts\install-backups\pre-upstream-265-vst3-layouts-20260811-133138`, then hash-verified after overlay; Windows Audio was restored and the installed VST self-test passed its bus-contract case. Manual real multichannel VST3 acceptance remains open.
- 2026-08-11: staged upstream draft #265 on `codex/upstream-265-vst3-layouts` from beta `9bf6974`. The existing `VSTPlugin:` grammar gains paired VST3 `Input`/`Output` layouts, both Editor row implementations preserve them opaquely, and VST2 ignores them at runtime. Review caught and fixed numeric `Input`/`Output` legacy parameter parsing, legacy non-finite parameter compatibility, and 6.1 candidate ordering; new tests prove Music-first/Cine-fallback. Local source/variant sync, HybridConv 1635, EditorLogic 3544, EngineOrchestration 1221, AudioRegression 31/31, AVX2 Editor self-test, and skin switch passed. Hosted run `31515643821` passed Pester, cppcheck, all six native variants, offscreen UI gates, packaging, and cross-variant comparison. `beta`/`main` remain unchanged; only a real multichannel VST3 manual test remains.
...
- 2026-08-09 CI correction: run `31320398601` failed only because Gallery required scrolling from compact Studio-derived `midnight`/`obsidian` skins. Replaced the skin-ID condition with actual content-overflow detection; AVX2 Editor rebuild, VST self-test, source/variant sync, and the full 20-skin gallery (5120 PNGs, zero warnings) passed locally. Fresh beta run `31322362583` passed Pester, cppcheck, all six native variants, offscreen UI gates, packaging, and cross-variant comparison.
- 2026-08-09 beta handoff: published prerelease `v2.36.0-beta.1` from CI-tested `c0c858d`, attaching validated portable AVX-512/AVX2 archives plus `SHA256SUMS.txt`; auto-detect installer intentionally omitted. Manual real microphone/capture and playback acceptance remains the promotion gate.
