...
- 2026-08-13: pushed the tree-identical #265/#271 implementation as `740a86e` on the isolated branch and manually dispatched hosted Build EqualizerAPO run `31711097531`, because the workflow only auto-runs for `main`/`SIMD_CI`. Await its full cross-variant/offscreen UI result, then run the real `Input Stereo Output 7.1` VST3 acceptance before any promotion. No active runtime changed.
