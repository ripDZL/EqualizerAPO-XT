...
- 2026-08-25: system-wide MSI route targets `C:\Program Files\EqualizerAPO-XT\<channel>` and preserves legacy/per-user installs. Core MSI commit `440bab7e` passed run `32844223431`. Corrected the fork endpoint and prerelease routing: stable Setup resolves this fork's `latest`; beta Setup pins its tag through `InstallerReleaseTag`. Red/green native URL coverage passes at 4310 checks; fresh beta-head CI and the approved `v2.42.3-beta.1` publish remain.
