...
- 2026-08-25: implemented the supported Velopack per-machine MSI route for the auto-detect installer. Release completeness and checksums require each channel's setup EXE, MSI, and feed; the front door verifies the MSI, requests UAC only for `msiexec`, and targets `C:\Program Files\EqualizerAPO-XT\<channel>`. Local native validation passes; CI Pester 5/package validation is pending.
